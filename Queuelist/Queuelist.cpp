...
int main()
{
    ...
    double p = 0.0;
    double q = 0.0;
    std::cout << "¬ведите веро€тность добавлени€ задачи P (0, 100): "; std::cin >> p;
    std::cout << std::endl << "¬ведите веро€тность извлечени€ задачи Q (0, 100): "; std::cin >> q;
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (randNumPush < p) {
            try {
                ...
                countPushExcept++;
                std::cout << "ќчередь полна, не могу добавить" << std::endl;
            }
        }
        ...
        if (randNumPop < q)
            ...
            tmp = q1.peektail();
            countPopSuccess++;
            std::cout << randNumPop << " < " << q << " изъ€ть " << tmp << std::endl;
        }
        catch (...) {
            countPopExcept++;
            std::cout << "ќчередь пуста, не могу изъ€ть" << std::endl;
        }
    }
